...
def build_detection_targets(rpn_rois, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    if remaining > 0:
        # Looks like we don't have enough gts to maintain the desired
        # balance. Reduce requirements and fill in the rest. This is
        # likely different from the Mask RCNN paper.

        # There is a small chance we have neither fg nor bg gts.
        if keep.shape[0] == 0:
...
def data_generator(dataset, config, shuffle=True, augment=False, augmentation=None,
    ...
    config: The model config object
    shuffle: If True, shuffles the gts before every epoch
    augment: (deprecated. Use augmentation instead). If true, apply random
        image augmentation. Currently, only horizontal flipping is offered.
    augmentation: Optional. An imgaug (https://github.com/aleju/imgaug) augmentation.
        For example, passing imgaug.augmenters.Fliplr(0.5) flips images
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers, class_weight=None,
        ...
        if class_weight is None:
            hist = self.keras_model.fit_generator(
                ...
                class_weight={0: 1.0, 1: 2.007, 2: 0.666}, 
                max_queue_size=100,
                workers=workers,
            ...
            hist = self.keras_model.fit_generator(
                ...
                initial_epoch=self.epoch,
                epochs=epochs,
                stepes_per_epoch=self.config.STEPS_PER_EPOCH,
                callbacks=callbacks,
                validation_data=val_generator,
                validation_steps=self.config.VALIDATION_STEPS,
                max_queue_size=100,
...
def compute_class_weight(class_weight):
    ...
    class_weights = dict(sorted(class_weight.items()))
    return class_weight
